...
function PrivateChat(props) {
  ...
  function OurChatJoinLeaveButtons(connected){
   ...
   function connect() {
     ...
     if(user.name) {
       ...
       stompClient.connect({}, function(frame) {
           ...
           stompClient.send("/app/addPrivateUser", {}, JSON.stringify({sender: user.name, type: 'JOIN', receiver: chatPartner,
}))
           stompClient.subscribe("/user/" + chatPartner + "/reply", function(messagePayload) {
             try {
   ...
   useEffect(() => {
     ...
     let newVar = props.location.state.chatPartner;
     // console.log(newVar);
     setChatPartner(newVar);
...
export default withCookies(PrivateChat);
